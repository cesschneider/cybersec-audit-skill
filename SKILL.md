...
- Linux (Ubuntu/Debian/RHEL/Arch) or WSL2
- Python 3.9+ (for bandit, pip-audit, semgrep)
- Node.js 18+ (optional, for npm audit / JS projects)
- Git (for tracking findings and auto-fix changes)
- Root or sudo access for system-wide installs (or use pip `--user`)

### Step 1: Install SAST Tools

Run this setup once per machine. Each tool is free and open-source.
...
**What it detects:** SQL injection, command injection, XSS, hardcoded secrets, insecure crypto, deserialization — for Python, JavaScript, TypeScript, Go, Java, Ruby, and 30+ other languages.
...
**What it detects:** Known CVEs in your Python packages (PyPI advisories).

#### snyk — Comprehensive commercial scanner (free tier available)
...
**What it detects:** The most comprehensive scanner — code vulnerabilities, dependency CVEs, container image issues, IaC misconfigurations, license compliance.
...
**What it detects:** Container image vulnerabilities, filesystem scans, IaC (Dockerfiles, Terraform, K8s), SBOM generation, license scanning.

### Step 2: Verify All Tools

```bash
echo "=== SAST Tools Available ==="
echo -n "semgrep: " && which semgrep && semgrep --version || echo " NOT INSTALLED"
echo -n "bandit:  " && which bandit && bandit --version || echo "  NOT INSTALLED"
echo -n "snyk:    " && which snyk && snyk --version || echo "    NOT INSTALLED"
echo -n "trivy:   " && which trivy && trivy --version || echo "   NOT INSTALLED"
echo -n "pip-audit: " && which pip-audit && pip-audit --version || echo "  NOT INSTALLED"
```

### Step 3: Quick Installation Script

Copy and paste this single command to install all tools at once:

```bash
pip install --upgrade semgrep bandit pip-audit
```

For the full setup including snyk and trivy:
...
# semgrep (comprehensive multi-language)
...
echo "Scan complete. Results in /tmp/*json"
```
